...
# - get_value(name): Get a value
# - set_value(name, value): Set a value
...
class CounterController < StimulusController
  ...
  # Increment the counter using the increment_value helper
  def increment
    increment_value(:count)
    # Note: update_display is called automatically via count_value_changed callback
  end

  # Decrement the counter using the decrement_value helper
  def decrement
    decrement_value(:count)
    # Note: update_display is called automatically via count_value_changed callback
  end

  # Reset the counter to 0 using the set_value helper
  def reset
    set_value(:count, 0)
    # Note: update_display is called automatically via count_value_changed callback
  end
